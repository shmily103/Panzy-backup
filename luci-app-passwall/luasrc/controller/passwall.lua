...
function index()
    ...
    if nixio.fs.access("/usr/sbin/haproxy") then
        entry({"admin", "services", "passwall", "haproxy"},
              cbi("passwall/haproxy"), _("Load Balancing"), 94).leaf = true
    end
    entry({"admin", "services", "passwall", "node_subscribe"},
    ...
    entry({"admin", "services", "passwall", "log"}, form("passwall/log"),
    ...
    entry({"admin", "services", "passwall", "update_rules"}, call("update_rules")).leaf =
        true
    entry({"admin", "services", "passwall", "luci_check"}, call("luci_check")).leaf =
        true
    entry({"admin", "services", "passwall", "luci_update"}, call("luci_update")).leaf =
        true
