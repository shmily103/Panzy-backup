...
s.extedit = d.build_url("admin", "services", "passwall", "node_config", "%s")
function s.create(e, t)
    local e = TypedSection.create(e, t)
    luci.http
        .redirect(d.build_url("admin", "services", "passwall", "node_config", e))
end

function s.remove(t, a)
    s.map.proceed = true
    s.map:del(a)
    luci.http.redirect(d.build_url("admin", "services", "passwall", "node_list"))
end
